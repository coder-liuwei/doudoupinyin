...
export default function PrintOnly() {
  ...
  const [data, setData] = useState<{ paragraphs: Paragraph[]; title: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // 1. 优先查 localStorage（用户已保存的）
    try {
      const schema = loadHistory();
      const found = schema.records.find((r: HistoryRecord) => r.id === id);
      if (found) {
        setData({ paragraphs: found.paragraphs, title: found.title });
        return;
      }
    } catch {
      // lib/history 尚未就绪或抛错，继续往下走 sessionStorage 路径
    }

    // 2. 否则查 sessionStorage（usePrint hook 临时存的）
    const temp = sessionStorage.getItem("pinyinPrince.print-temp");
    if (temp) {
      try {
        const parsed = JSON.parse(temp);
        if (parsed && parsed.id === id && Array.isArray(parsed.paragraphs)) {
          setData({ paragraphs: parsed.paragraphs, title: parsed.title ?? "" });
          return;
        }
      } catch {
        // ignore 解析错误
